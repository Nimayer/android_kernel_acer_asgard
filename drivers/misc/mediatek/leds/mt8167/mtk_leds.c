...
//static unsigned int charge_led_gpio;
static unsigned int charge_led_gpio_red;
static unsigned int charge_led_gpio_green;
static unsigned int charge_led_gpio_red_uart;
static unsigned int charge_led_gpio_green_uart;
	...
	if (pled_dtsi == NULL) {
		...
		for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
			...
			if (!led_node) {
				 ...
				 */
#if 0					
			/*===Begin Neostra lixiaolong add for charge led drv 20161219*/

			if(strcmp(pled_dtsi[i].name, "red")== 0){
				charge_led_gpio = of_get_named_gpio(led_node, "led_gpios", 0);
				ret = gpio_request(charge_led_gpio, "led_gpios");
				if (ret)
				printk("brucelee  222 cannot find gpio from dts %d\n", charge_led_gpio);

				gpio_direction_output(charge_led_gpio, 1); //mod neostra chenlijun 20170215
			}
			/*===End Neostra lixiaolong add for charge led drv 20161219*/
#else

							if(strcmp(pled_dtsi[i].name, "red")== 0){
								printk("brucelee  333\n ");
				charge_led_gpio_red = of_get_named_gpio(led_node, "led_gpios_red", 0);
				ret = gpio_request(charge_led_gpio_red, "led_gpios_red");
				if (ret)
				printk("brucelee  333 cannot find gpio from dts %d\n", charge_led_gpio_red);


				charge_led_gpio_red_uart= of_get_named_gpio(led_node, "led_gpios_red", 1);
				ret1 = gpio_request(charge_led_gpio_red_uart, "led_gpios_red");
				if (ret1)
				printk("brucelee  333 cannot find gpio from dts %d\n", charge_led_gpio_red_uart);
								...
								if(strcmp(pled_dtsi[i].name, "green")== 0){
									printk("brucelee  222 \n ");
				charge_led_gpio_green= of_get_named_gpio(led_node, "led_gpios_green", 0);
				ret = gpio_request(charge_led_gpio_green, "led_gpios_green");
				if (ret)
				printk("brucelee  222 cannot find gpio from dts %d\n", charge_led_gpio_green);

				charge_led_gpio_green_uart= of_get_named_gpio(led_node, "led_gpios_green", 1);
				ret1 = gpio_request(charge_led_gpio_green_uart, "led_gpios_green");
				if (ret1)
				printk("brucelee  222 cannot find gpio from dts %d\n", charge_led_gpio_green_uart);
...
#if 0	/*===Begin Neostra lixiaolong add for charge led drv 20161219*/	
...
//#else
		...
		{
			gpio_direction_output(charge_led_gpio, 1);
			printk("=====brucelee== MT65XX_LED_MODE_GPIO red\n");
		}
		else if(strcmp(cust->name, "green")==0)
		{
			gpio_direction_output(charge_led_gpio, 0);
			printk("=====brucelee== MT65XX_LED_MODE_GPIO green\n");
		}
		return 1;
#else
		printk("brightness_set_cust:go GPIO mode!!!!!\n");
   if(strcmp(cust->name, "red")== 0){
   	if(level==0){
		gpio_direction_output(charge_led_gpio_red_uart, 0);
		gpio_direction_output(charge_led_gpio_red, 0);}
		else{
	gpio_direction_output(charge_led_gpio_green, 0);
		gpio_direction_output(charge_led_gpio_red, 1);
		
		gpio_direction_output(charge_led_gpio_green_uart, 0);
		gpio_direction_output(charge_led_gpio_red_uart, 1);
    ...
    if(strcmp(cust->name, "green")==0){
		...
		gpio_direction_output(charge_led_gpio_green, 1);
		gpio_direction_output(charge_led_gpio_red, 0);
		
			gpio_direction_output(charge_led_gpio_green_uart, 1);
		gpio_direction_output(charge_led_gpio_red_uart, 0);
		...
		return 1;
#endif /*===End Neostra lixiaolong add for charge led drv 20161219*/
