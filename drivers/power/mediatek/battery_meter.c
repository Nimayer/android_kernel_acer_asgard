...
extern int read_battery_id(void);
...
//neostra chenlijun mod 20170105
void fgauge_get_profile_id(void)
{
	
	 int r = read_battery_id();
	 printk("clj1 read_battery_id debug %d\n",r);
	
	
		g_fg_battery_id = r > 0 ? 1 : 0;
}
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	idx = 0;
	 if (read_battery_id() == 1){
		 while (!of_property_read_u32_index(np, "batt_temperature_table_tcl", idx, &addr)) {
			 idx++;
			 if (!of_property_read_u32_index(np, "batt_temperature_table_tcl", idx, &val)) {
				 battery_log(BAT_LOG_CRTI, "batt_temperature_table_tcl: addr: %d, val: %d\n",addr, val);
			 }
...
__batt_meter_parse_node(np, "battery_profile_t0_num", &num);
	  if (read_battery_id() == 1)
	  	{
	  	__batt_meter_parse_table(np, "battery_profile_t0_tcl",
...
__batt_meter_parse_node(np, "battery_profile_t1_num", &num);
	if (read_battery_id() == 1)
	{__batt_meter_parse_table(np, "battery_profile_t1_tcl",
...
__batt_meter_parse_node(np, "battery_profile_t2_num", &num);
	if (read_battery_id() == 1)
	{__batt_meter_parse_table(np, "battery_profile_t2_tcl",
...
__batt_meter_parse_node(np, "battery_profile_t3_num", &num);
	if (read_battery_id() == 1)
		{
	__batt_meter_parse_table(np, "battery_profile_t3_tcl",
...
__batt_meter_parse_node(np, "r_profile_t0_num",	&num);
	if (read_battery_id() == 1){__batt_meter_parse_table(np, "r_profile_t0_tcl",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t0), num);}else{
...
__batt_meter_parse_node(np, "r_profile_t1_num",	&num);
	if (read_battery_id() == 1){__batt_meter_parse_table(np, "r_profile_t1_tcl",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t1), num);}else{
...
__batt_meter_parse_node(np, "r_profile_t2_num",	&num);
	if (read_battery_id() == 1){__batt_meter_parse_table(np, "r_profile_t2_tcl",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t2), num);}else{
...
__batt_meter_parse_node(np, "r_profile_t3_num",	&num);
	if (read_battery_id() == 1){__batt_meter_parse_table(np, "r_profile_t3_tcl",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t3), num);}else{
...
//neostra chenlijun add 20170222
//#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT   
#if 1
int BattThermistorConverTemp(int Res)
{ 
 //neostra rm chenlijun 20170414 for auto turn off device when palying spky about 20 minuties
	int i = 0;
	int RES1 = 0, RES2 = 0;
...
//neostra add chenlijun 20170106
signed int g_Q_MAX_POS_50[] = {6298,6000};
signed int g_Q_MAX_POS_25[] = {6287,6096 };
signed int g_Q_MAX_POS_0[] = {6028,5706};
signed int g_Q_MAX_NEG_10[] = {5615,5661};
signed int g_Q_MAX_POS_50_H_CURRENT[] = {6228,5935};
signed int g_Q_MAX_POS_25_H_CURRENT[] = {6225,6033};
signed int g_Q_MAX_POS_0_H_CURRENT[] = {5917,5612};
signed int g_Q_MAX_NEG_10_H_CURRENT[] = {5186,5416};
...
signed int battery_meter_initial(void)
{
	...
	if (meter_initilized == KAL_FALSE) {
		//neostra chenlijun mod 20170222
//#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
#if 1
...
//Neostra huangjianlong add 20170320
		...
		neostra_add_hw_info("battery", (g_fg_battery_id == 1)? "tcl":"haopeng", 0);
#endif
...
//neostra chenlijun 20170208 add file node for battery ID,1is TCL,esle 0 is haopeng 
static ssize_t show_BATID_Pin(struct device *dev, struct device_attribute *attr, char *buf)
{
	signed int ret = 0;
	char ids[2][10]={"tcl","haopeng"};
	ret=read_battery_id();
	bm_print(BM_LOG_CRTI, "battery id: %d\n", ret);
	//ids = ret > 0 ? "tcl" : "haopeng";
	if (ret > 0)
...
static DEVICE_ATTR(BATID_Pin, 0664, show_BATID_Pin, NULL);
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	//neostra chenlijun 20170208 add file node for battery ID,1is TCL,esle 0 is haopeng 
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_BATID_Pin);
