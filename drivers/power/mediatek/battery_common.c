...
static unsigned int battery_id_gpio;
...
//Neostra huangxiaohui add 20160918
#define Battery_charge_max_vol 4210
...
static void mt_battery_notify_TotalChargingTime_check(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		    ...
		    g_BatteryNotifyCode);
              ...
              battery_log(BAT_LOG_CRTI,"hxh total_charging_time=%d, battery_level=%d,ICharging_current=%d,bat_full=%d \n",
                                  BMT_status.total_charging_time, BMT_status.UI_SOC, BMT_status.ICharging, BMT_status.bat_full);
       //Begin Neostra huangxiaohui add 20160905
       ...
       if(((BMT_status.total_charging_time >= MAX_CHAREING_NOT_FULL_TIME) && (BMT_status.bat_in_recharging_state == KAL_FALSE) && (BMT_status.UI_SOC < 100)) 
               || ((BMT_status.bat_vol > Battery_charge_max_vol) || (BMT_status.bat_vol < Battery_charge_min_vol) )
               /*|| ( (BMT_status.bat_exist) && (upmu_is_chr_det() == KAL_TRUE) && (BMT_status.bat_in_recharging_state == KAL_FALSE) && (BMT_status.bat_full != KAL_TRUE)  && (BMT_status.ICharging == 0)   )*/                
         )
       //if(((BMT_status.total_charging_time >= 120)&& (BMT_status.UI_SOC < 100)))//test
       {
               g_BatteryNotifyCode |= 0x0012;//the new value:0x0012 is for show_charger_badbattery_logo
               battery_log(BAT_LOG_CRTI,"hxh total_charging_time=%d, battery_level=%d,ICharging_current=%d,Charger_exist=%d \n",
                                  BMT_status.total_charging_time, BMT_status.UI_SOC, BMT_status.ICharging, upmu_is_chr_det());
                                          
       }
       else
       {
               g_BatteryNotifyCode &= ~(0x0012);
       }
       //End Neostra huangxiaohui add 20160905
...
/***Begin Neostra lixiaolong add for read battery id for battery compatible***/	
int read_battery_id(void)
{
	return gpio_get_value(battery_id_gpio);
}
/***End Neostra lixiaolong add for read battery id for battery compatible***/
...
/***Begin Neostra lixiaolong add for read battery id for battery compatible***/	
	battery_id_gpio= of_get_named_gpio(np, "battery_id", 0);
	ret = gpio_request(battery_id_gpio, "battery_id");
	if (ret)
		printk("===brucelee=== cannot find batttery id gpio from dts %d\n", battery_id_gpio);
	
	gpio_direction_input(battery_id_gpio);

	printk("==clj brucelee== battery id=== %d\n",read_battery_id());
/***End Neostra lixiaolong add for read battery id for battery compatible***/
